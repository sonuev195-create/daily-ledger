...
export function BulkItemEdit({ onClose, onSaved }: BulkItemEditProps) {
  ...
  // Same rate selection
  const [sameRateMode, setSameRateMode] = useState(false);
  const [sameRateSelected, setSameRateSelected] = useState<Set<number>>(new Set());
  const [sameRateValue, setSameRateValue] = useState('');
  ...
  const loadData = async () => {
    ...
    const stockMap: Record<string, { primary: number; secondary: number; rate: number; count: number }> = {};
    (batchesData || []).forEach(b => {
      if (!stockMap[b.item_id]) stockMap[b.item_id] = { primary: 0, secondary: 0, rate: 0, count: 0 };
      stockMap[b.item_id].primary += Number(b.primary_quantity);
      stockMap[b.item_id].secondary += Number(b.secondary_quantity);
      stockMap[b.item_id].rate += Number(b.purchase_rate);
      stockMap[b.item_id].count += 1;
    });

    setItems((itemsData || []).map(item => {
      const avgRate = stockMap[item.id]?.count ? stockMap[item.id].rate / stockMap[item.id].count : 0;
      return {
        id: item.id,
        name: item.name,
        paperBillName: (item as any).paper_bill_name || '',
        ...
        purchaseRate: avgRate,
        newName: item.name,
        newPaperBillName: (item as any).paper_bill_name || '',
        ...
        newPurchaseRate: String(avgRate || 0),
        changed: false,
      };
    }));
    setLoading(false);
  ...
  const updateField = (index: number, field: keyof BulkItem, value: string) => {
    setItems(prev => {
      ...
      item.changed = item.newName !== item.name ||
        item.newPaperBillName !== item.paperBillName ||
        ...
        item.newPurchaseRate !== String(item.purchaseRate);
      updated[index] = item;
      return updated;
    });
  };
  ...
  const applySameRate = () => {
    const rate = parseFloat(sameRateValue);
    if (isNaN(rate)) { toast.error('Enter a valid rate'); return; }
    setItems(prev => prev.map((item, i) => {
      if (!sameRateSelected.has(i)) return item;
      const updated = { ...item, newSellingPrice: String(rate) };
      updated.changed = updated.newName !== updated.name ||
        updated.newPaperBillName !== updated.paperBillName ||
        updated.newSellingPrice !== String(updated.sellingPrice) ||
        updated.newCategoryId !== (updated.categoryId || '') ||
        updated.newBatchPreference !== updated.batchPreference ||
        updated.newPrimaryStock !== String(updated.primaryStock) ||
        updated.newSecondaryStock !== String(updated.secondaryStock) ||
        updated.newPurchaseRate !== String(updated.purchaseRate);
      return updated;
    }));
    setSameRateMode(false);
    setSameRateSelected(new Set());
    setSameRateValue('');
    toast.success('Selling price updated for selected items');
  };
  ...
  const handleSaveAll = async () => {
    ...
    try {
      for (const item of changedItems) {
        // Update item master
        if (item.newName !== item.name || item.newPaperBillName !== item.paperBillName ||
            item.newSellingPrice !== String(item.sellingPrice) ||
            item.newCategoryId !== (item.categoryId || '') || item.newBatchPreference !== item.batchPreference) {
          await supabase.from('items').update({
            name: item.newName,
            paper_bill_name: item.newPaperBillName || null,
        ...
        if (newPrimary !== item.primaryStock || newSecondary !== item.secondaryStock) {
          ...
          if (batches && batches.length > 0) {
            ...
            await supabase.from('batches').update({
              ...
              purchase_rate: newRate || Number(openBatch.purchase_rate),
              batch_number: batchName,
            }).eq('id', openBatch.id);
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => { setSameRateMode(!sameRateMode); setSameRateSelected(new Set()); }} className="gap-1 text-xs h-7">
            {sameRateMode ? 'Cancel' : 'Same Rate'}
          </Button>
      ...
      {sameRateMode && (
        <div className="px-4 py-2 bg-accent/10 border-b border-accent/20 flex items-center gap-2">
          <span className="text-xs text-muted-foreground">Select items, then set price:</span>
          <Input type="number" value={sameRateValue} onChange={e => setSameRateValue(e.target.value)} className="h-7 w-24 text-xs" placeholder="Rate" />
          <Button size="sm" className="h-7 text-xs" onClick={applySameRate} disabled={sameRateSelected.size === 0}>
            Apply to {sameRateSelected.size}
          </Button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="min-w-[900px]">
          ...
          {items.map((item, i) => (
            <div key={item.id}>
              <div className={cn(
                "grid grid-cols-[minmax(130px,1fr)_100px_90px_65px_65px_65px_65px_80px_36px] gap-1 px-3 py-1 border-b border-border/30 items-center",
                item.changed && "bg-accent/5"
              )}>
                {sameRateMode && (
                  <input type="checkbox" checked={sameRateSelected.has(i)}
                    onChange={() => setSameRateSelected(prev => {
                ...
                />
                <Input
                  value={item.newPaperBillName}
                  onChange={e => updateField(i, 'newPaperBillName', e.target.value)}
                  className="h-7 text-xs px-1.5"
                ...
                <Input
                  type="number"
                  value={item.newPurchaseRate}
                  onChange={e => updateField(i, 'newPurchaseRate', e.target.value)}
                  className="h-7 text-xs px-1.5"
                />
                <Input
                  type="number"
